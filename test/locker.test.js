...
contract("LiquidLocker", async accounts => {
    ...
    beforeEach(async() => {
        ...
        const predicted = await factory.predictLockerAddrss(
            counter,
            factory.address,
            target
        );
        ...
        const counterFirst = await factory.lockerCount();

        assert.equal(
            parseInt(counterFirst),
            parseInt(parseInt(counter) + 1)
        );
        ...
        const counterAgain = await factory.lockerCount();
        ...
        const counterAfter = await factory.lockerCount();

        assert.equal(
            parseInt(counterAgain) + 1,
            parseInt(counterAfter)
        );
        ...
        const predictedAgain = await factory.predictLockerAddrss(
            counterAgain,
            factory.address,
            target
        );

        indexer = counterAgain;
    ...
    describe("LiquidLocker", () => {
        ...
        describe("Deactivate Loan and refund", () => {
            ...
            it("should allow to retrieve tokens only if locker is disabled by owner", async () => {
