    ...
    describe("LiquidLockerLifecycle", () => {
        ...
        describe("Finish/Liquidate Loan", () => {
            ...
            it("Refunds for multiple contributors", async () => {
                ...
                bobStartBal = await usdc.balanceOf(bob);
                randStartBal = await usdc.balanceOf(random);
                ...
                await timeMachine.advanceTimeAndBlock(86400 * 5); //ran this with lower values for early disable/0 for immediate disable. All work as intended

                bobMiddleBal = await usdc.balanceOf(bob);
                randMiddleBal = await usdc.balanceOf(random);
                ...
                await locker.refundDueDisabled(
                    bob,
                    {
                        from : bob
                    }
                ...
                assert.equal(lockerOwner, ZERO_ADDRESS);
                ...
                towner = await nft.ownerOf(tokenId);
            ...
            it("Refund after single contributor takes over", async () => {
                ...
                await timeMachine.advanceTimeAndBlock(86400 * 5);
                ...
                assert.equal(bobBal.toString(), bobStartBal.toString());
                assert.equal(randBal.toString(), randMiddleBal.toString());

                lockerOwner = (await locker.globals()).lockerOwner;

                assert.equal(lockerOwner, alice);
                ...
                assert.equal(locker.address, towner);
            ...
            it("Test Refund on rescue locker disable", async () => {
                ...
                const bal1 = await usdc.balanceOf(alice);
                ...
                await timeMachine.advanceTimeAndBlock(86400 * 10);
            ...
            it("Payoff on last days possible", async () => {
                ...
                await factory.createLiquidLocker(
                    ...
                    tokens(600),
                    tokens(600),
                    86400 * 300, //expect .2 token payoff needed per day with these params
                    10,
                    usdc.address,
                    {from: alice, gas: 3000000}
                );
                ...
                await compareDueTimeToBlockTime(locker);

                await advanceTimeAndBlock(86400 * 7.5); //because one day already covered 7.5 instead of 6.5, nextduetime is already on tomorrow when enable or payback gets called
                ...
                debug("Penalties: " + penalties.toString());

                assert.equal(penalties.toString(), "24000000000000000000");

                await advanceTimeAndBlock(86400 * 7.5);
                ...
                assert.equal(penalties.toString(), "48000000000000000000");
            ...
            it("Due date is calculated as intended over multiple payments", async () => {
                ...
                await timeMachine.advanceTimeAndBlock(86400 * 6);
            ...
            it("Contributing on your own loan doe not break things", async () => {
                ...
                const bal0 = await usdc.balanceOf(alice);
                ...
                const bal2 = await usdc.balanceOf(alice);
                ...
                assert.equal("0x910c094b260c8b1493497a8d6A780f0A48f0b9E7",towner);
            ...
            it("Very long loan", async () => {
                ...
                await timeMachine.advanceTimeAndBlock(86400 * 3650); //going much bigger than this is impossible because internals of ganache exceed js number limit. Only 10 is years instead of 100 into future
            ...
            it("Very short loan", async () => {
                ...
                await factory.createLiquidLocker(
                    ...
                    tokens(600),
                    tokens(600),
                    86400 * 5 + 300, //5 minute loan, payment time is the actual final date, not how many days since enable
                    10,
                    usdc.address,
                    {from: alice, gas: 3000000}
                );
            ...
            it("penalty amount is as expected", async () => {
                ...
                for(let i = 0; i < 10; i++) {
                    ...
                    await timeMachine.advanceTimeAndBlock(86400 * 1);
            ...
            it("Interest collection works as predicted", async () => {
                ...
                await locker.claimInterestPublic(
                    {
                        from : bob
                    }
                )
            ...
            it("Penalty split test payoff", async () => {
                nft = await NFT721.new();
                ...
                actualBalance = await usdc.balanceOf(locker.address);
                ...
                claimable = await locker.claimableBalance();
                actualBalance = await usdc.balanceOf(locker.address);
                debug("Claimable after pen payback " + fromWei(claimable.toString()));
                debug("actual tokens in contract " + fromWei(actualBalance.toString()));
                remaining = await locker.remainingBalance();
                debug("Remaining after pen payback " + fromWei(remaining.toString()));
                debug("\n");
            ...
            it("Penalty liquidate not enough test", async () => {
                ...
                await timeMachine.advanceTimeAndBlock(86400 * 12);

                penalties = await locker.penaltiesBalance();

                console.log("Total Penalties " + penalties.toString());
            ...
            it("Penalty liquidate with enough test", async () => {
                ...
                await timeMachine.advanceTimeAndBlock(86400 * 280);
            ...
            it("Decrease Time during contributions", async () => {
                ...
                assert.equal(time.toNumber(), 86400 * 30);
